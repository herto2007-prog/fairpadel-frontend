...
type Tab = 'resumen' | 'editar' | 'inscripciones' | 'finanzas' | 'sorteo' | 'canchas' | 'pelotas' | 'ayudantes' | 'acreditacion' | 'reportes' | 'dashboard';
...
function ResumenTab({ tournament, stats, onRefresh }: { tournament: Tournament; stats: TournamentStats | null; onRefresh: () => Promise<void> }) {
  ...
  const { hasRole: resumenHasRole } = useAuthStore();
  const isAdmin = resumenHasRole('admin');
  ...
  // Default seed targets from seed-torneo-masivo.ts
  ...
  // Initialize seedConfig from tournament categories
  useEffect(() => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Panel Seed Test Data (Admin only) */}
      {isAdmin && stats?.categorias && stats.categorias.length > 0 && (
