...
const SedeSelector: React.FC<SedeSelectorProps> = ({ selectedSedeId, onSelect, ciudad }) => {
  ...
  if (sedes.length === 0) {
    return (
      <div className="text-center py-6 text-light-secondary">
        <MapPin className="h-8 w-8 mx-auto mb-2 text-dark-border" />
        <p className="text-sm">
          {ciudad
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-light-secondary">
        ...
        {ciudad && (
          <span className="text-primary-500 font-medium"> Mostrando sedes en {ciudad}.</span>
        )}
      </p>
            ...
            <button
              ...
              className={`text-left p-4 rounded-lg border-2 transition-all ${
                isSelected
                  ...
                  : 'border-dark-border hover:border-dark-border hover:bg-dark-hover'
              }`}
            >
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3 flex-1">
                  {sede.logoUrl ? (
                    ...
                    <div className="h-10 w-10 rounded-lg bg-primary-500/20 flex items-center justify-center text-primary-400 font-bold">
                      {sede.nombre.charAt(0)}
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-light-text truncate">{sede.nombre}</p>
                    <div className="flex items-center gap-1 text-sm text-light-secondary mt-0.5">
                    ...
                    {sede.telefono && (
                      <div className="flex items-center gap-1 text-xs text-light-secondary mt-0.5">
                        <Phone className="h-3 w-3" />
                        {sede.telefono}
                      </div>
                    )}
                    <div className="flex items-center gap-1 text-xs text-light-secondary mt-1">
